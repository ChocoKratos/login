...
function validatePhone(phone) {
  // Panamá: 6xxx-xxxx (celular) o 2xxx-xxxx / 3xxx-xxxx (fijo)
  return /^[23456789]\d{3}-?\d{4}$/.test(phone.replace(/\s/g, ""));
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!firstName || !lastName || !email || !birthDate || !phone || !cedula || !password || !confirmPassword)
    ...
    if (!validatePhone(phone))
      return { statusCode: 400, headers, body: JSON.stringify({ ok: false, error: "Teléfono inválido. Formato panameño: 6xxx-xxxx." }) };
    ...
    const totp   = new OTPAuth.TOTP({ issuer: "LoginPremium", label: key, algorithm: "SHA1", digits: 6, period: 30, secret });
    ...
    console.log("Saved:", key, "status:", saveStatus);
